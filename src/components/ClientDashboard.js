...
const ClientDashboard = () => {
  ...
  // Geo API to get city and state using the provided API key
  // Fetch geolocation data based on the user's IP address
  // Fetch geo data based on the user's IP address
  const getGeoData = async () => {
    try {
      const response = await axios.get(
        "http://api.ipstack.com/check?access_key=45d26015e0994bf8aeaac6bcfdee3abd"
      );

      // Extract city and state from the response
      const city = response.data.city;
      const state = response.data.region_name;
  ...
  // Update the state and city based on geo data
  const setGeoLocationData = async () => {
    const { city, state } = await getGeoData();
  ...
  useEffect(() => {
    // Automatically set the state and city based on geolocation when the component mounts
    setGeoLocationData();
  }, []);
  ...
  // Assuming these states and setters are defined
